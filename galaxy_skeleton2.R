...
galaxies <- subset(galaxies, subset = !is.na(galaxies$name))
...
#Problem 3----
#I create a new column to specify whether the galaxy is big or small
# i categorize the galaxies by saying small galaxies has a a_26 less than 1 
#and the rest are big
galaxies <- galaxies %>% 
  mutate(size_category = if_else(a_26 < 1 , "small", "big"))

#Make a plot to vizualize the frequenzy of big and small galaxies:
galaxies %>% 
...
#Explanation fro why smaller objects might be under represented in the sample:

#Observational Limitations: Smaller galaxies may be harder to detect and observe 
#due to their size, distance, and brightness, which might lead to 
